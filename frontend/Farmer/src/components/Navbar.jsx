    ...
    <header className="max-w-screen-2x1 container mx-auto fixed top-0 right-0 transition-all duration-300 ease-in-out">
      <div className={`navbar px-4 ${isSticky ? "shadow-md bg-base-100 transition-all duration-300 ease-in-out" : ""}`}>
        ...
        <div className="navbar-end">
          ...
          {/* cart items */}
          <Link to="cart-page">
          <label tabIndex={0} role="button" className="btn btn-ghost btn-circle mr-4 lg:flex hidden items-center justify_center">
          ...
          <button onClick={()=>document.getElementById('my_modal_3').showModal()} 
           className="btn bg-green rounded-full px-3 py-2 text-white flex items-center gap-7 text-lg" style={{ paddingRight: '35px' }}>
             <FiUsers/>  Login  
              </button>
              <Modal/>
